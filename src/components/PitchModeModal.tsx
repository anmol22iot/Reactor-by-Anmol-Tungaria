      ...
      <div className="bg-[#0f1420] border-2 border-sky-500/50 rounded-2xl max-w-2xl w-full p-6 shadow-2xl shadow-sky-500/10 max-h-[90vh] overflow-y-auto">
        ...
        <div className="mt-5 space-y-4 text-xs leading-relaxed">
          ...
          <div className="bg-[#141b2d] p-3.5 rounded-xl border border-[#222f4c]">
            <div className="flex items-center gap-2 text-sky-300 font-bold text-sm mb-1">
              <span className="w-5 h-5 rounded-full bg-sky-500/20 flex items-center justify-center text-xs text-sky-200 font-bold">1</span>
              Minute 1: The Critical Mission & Fragile Optimism
            </div>
            <p className="text-slate-200 font-medium leading-relaxed">
              Show the Active Operational Goal: <strong className="text-white">"Fulfill 500 orders by 6:00 PM with ₹15,000 budget."</strong> The engine scans the warehouse, finds 415 units (83%), detects an 85-unit shortage, and places an order with Supplier A for ₹9,200. Everything looks standard until...
            </p>
          </div>

          {/* Section 2 */}
          <div className="bg-rose-950/30 p-3.5 rounded-xl border border-rose-700/60">
            <div className="flex items-center gap-2 text-rose-200 font-bold text-sm mb-1">
              <span className="w-5 h-5 rounded-full bg-rose-500/20 flex items-center justify-center text-xs text-rose-200 font-bold">2</span>
              Minute 2: Real-World Failure & Autonomous Resilience
            </div>
            <p className="text-slate-200 font-medium leading-relaxed">
              <strong className="text-rose-200 font-bold">Supplier A REJECTS the PO (HTTP 422 capacity constraint).</strong> Emphasize to the judges: <em className="text-slate-100">"A static script or conventional chatbot crashes here and alerts humans. REACTOR treats failure as an expected state condition."</em> In under 3 seconds, it blacklists Supplier A, recomputes an optimal multi-supplier split (Supplier B: 50u @ ₹8,000 + Supplier C: 35u @ ₹5,800 = ₹13,800 &lt; ₹15,000 cap), and executes parallel orders automatically with 0s human lag!
            </p>
          </div>

          {/* Section 3 */}
          <div className="bg-emerald-950/30 p-3.5 rounded-xl border border-emerald-700/60">
            <div className="flex items-center gap-2 text-emerald-200 font-bold text-sm mb-1">
              <span className="w-5 h-5 rounded-full bg-emerald-500/20 flex items-center justify-center text-xs text-emerald-200 font-bold">3</span>
              Minute 3: Independent Outcome Verification
            </div>
            <p className="text-slate-200 font-medium leading-relaxed">
              Point to the <strong className="text-emerald-200 font-bold">Independent Outcome Verification card</strong>. <em className="text-slate-100">"REACTOR does not grade its own homework."</em> An isolated verifier independently validates external ERP database truth: 500/500 orders protected, spend ledger verified at ₹13,800, carrier SLA arrival 17:30 &lt; 18:00 cutoff.
            </p>
          </div>

          {/* Section 4 */}
          <div className="bg-amber-950/30 p-3.5 rounded-xl border border-amber-700/60">
            ...
            <p className="text-slate-200 font-medium leading-relaxed">
              Show the <strong className="text-amber-200 font-bold">Chaos Mode selector</strong>: switch to <strong className="text-white">"Budget Cap Tightening (₹12k)"</strong> to prove that when an autonomous action exceeds authorized spending thresholds, REACTOR automatically pauses for human supervisor authorization!
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between pt-4 border-t border-[#1a233a]">
          <span className="text-[11px] text-slate-300 font-mono font-medium">
            Key Metric: <strong className="text-emerald-300 font-bold">100% Volume Protected · ₹1,200 Surplus</strong>
          </span>
          <div className="flex space-x-2">
            <button
              onClick={onClose}
              className="px-4 py-2 rounded-lg bg-[#141b2d] hover:bg-[#1a233a] text-slate-200 text-xs font-semibold transition cursor-pointer"
            >
              Close
            </button>
            <button
